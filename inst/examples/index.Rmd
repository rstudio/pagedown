...
The default CSS selector is `".level1 p:not(.caption)"`. Line numbering is deactivated for display math environments.

If your template relies on custom CSS files, the CSS `line-height` property must have a value different than `"normal"`. For example:
